...
public class ClassicalFutureByMinStrategy extends AbstractStrategy implements IStrategy {
	...
	public void init() {
		...
		iMap.put(K, 0.6D);
		iMap.put(OPEN_HANDS_INDEX, 3D);
		iMap.put(Context.HISTORICAL_RANGE, 18);
		contextMap.put(Variety.RB, iMap);
	...
	public void freq30M(Account acc, Bar bar, Variety variety, Security security, List<Contract> list) {
		...
		for (Entry<Variety, Map<String, Object>> entry : contextMap.entrySet()) {
			...
			handleFreq30M(acc, bar, variety, varietyMap, security, list);
		}
	}
	
	private void handleFreq30M(Account acc, Bar bar, Variety variety, Map<String, Object> params, Security security, List<Contract> contractList) {
		...
		log.info(now() + " 30M:" + mainSecurity.getName() + " Buy line:" + buyLine + ", Sell line:" + sellLine);
		String text = now() + " 30M:" + mainSecurity.getName() + " Buy line:" + buyLine + ", Sell line:" + sellLine;
		try {
